...
export function AuthCard() {
  ...
  return (
    <main className={`${pageBackground} grid place-items-center p-6`}>
      <Card
        className="w-full max-w-115 rounded-[28px] border-slate-400/30 bg-slate-950/80 p-7 text-slate-200 shadow-[0_24px_80px_rgba(0,0,0,0.35)] md:p-10"
        aria-labelledby="auth-title"
      >
        <CardHeader className="p-0">
          <p className={eyebrow}>Document Search Platform</p>
          <CardTitle className={displayTitle} id="auth-title">
            Start with your email
          </CardTitle>
          <CardDescription className="my-4.5 mb-7 text-slate-300">
            This demo stores your email locally to emulate authentication before
            document upload.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="p-0">
          <form className="grid gap-3" onSubmit={handleEmailSubmit} noValidate>
            <label className="font-bold text-slate-50" htmlFor="email">
              Email address
            </label>
            <Input
