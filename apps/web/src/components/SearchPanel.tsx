...
export function SearchPanel() {
  ...
  return (
    <Card className={`${contentWidth} ${glassCard} mt-5 box-border rounded-3xl p-6`} aria-labelledby="search-title">
      <CardHeader className="p-0">
        <CardTitle className="text-2xl" id="search-title">Search indexed documents</CardTitle>
        <p className="mt-1.5 text-slate-300">Search uses OpenSearch fuzziness and returns text highlights from matching documents.</p>
      </CardHeader>
      <CardContent className="p-0">
        <form className="mt-[18px] flex gap-3 max-md:flex-col max-md:items-stretch" onSubmit={handleSearchSubmit}>
          <Input
      ...
      {indexedDocumentsCount === 0 && documents.length > 0 ? (
        <p className="mt-3.5 text-slate-300">No indexed documents are searchable yet.</p>
      ) : null}
      ...
      {searchError ? <p className="mt-3.5 text-red-700">{searchError}</p> : null}
      {showEmptySearchResults ? (
        <p className="mt-3.5 box-border rounded-2xl border border-dashed border-slate-400/40 bg-slate-950/50 px-4 py-3.5 text-slate-300">No indexed matches found. Try words from the document text or filename.</p>
      ) : null}
      {searchResults.length > 0 ? (
        <div className="mt-[18px] grid gap-3">
          {searchResults.map((result) => (
            <article className={`${glassCard} rounded-[18px] p-4 shadow-none`} key={result.documentId}>
              <h3 className="mb-2.5 font-semibold">{result.userFilename}</h3>
              <div className="grid gap-2">
                {result.highlights.map((highlight, index) => (
                  <p className="m-0 text-slate-300" key={`${result.documentId}-${index}`}>{renderHighlight(highlight)}</p>
                ))}
              </div>
            </article>
          ))}
